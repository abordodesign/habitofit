...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      let result = await supabaseAdmin.from(table).select('*').order('id', { ascending: false })
      if (result.error && isMissingRelation(result.error) && table !== tables[1]) {
    ...
    if (req.method === 'POST') {
      ...
      let result = await supabaseAdmin.from(table).insert(payload).select('*').single()
      if (result.error && isMissingRelation(result.error) && table !== tables[1]) {
    ...
    if (req.method === 'PUT') {
      ...
      const payload =
        table === 'notifications'
          ? { title: titulo, body: descricao, image: imagem }
          : { titulo, descricao, imagem }
      let result = await supabaseAdmin.from(table).update(payload).eq('id', id).select('*').single()
      if (result.error && isMissingRelation(result.error) && table !== tables[1]) {
        const fallbackPayload =
          tables[1] === 'notifications'
            ? { title: titulo, body: descricao, image: imagem }
    ...
    if (req.method === 'DELETE') {
      const { id } = req.body || {}
      let result = await supabaseAdmin.from(table).delete().eq('id', id)
      if (result.error && isMissingRelation(result.error) && table !== tables[1]) {
